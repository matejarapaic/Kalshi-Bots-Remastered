...
4b. **Subscription watchdog:** a background task (every `STALE_BOOK_S`) recovers *structurally broken* subscriptions on a live connection — a book that never received its snapshot (subscribe raced the market's WS availability at window open, the cmd errored, or the snapshot was lost; observed live 2026-07-30: connected+subscribed 9 minutes with no book while REST showed a 1c spread) or one stuck in `seq_gap`. Recovery = `get_snapshot` when a `sid` exists, full resubscribe otherwise, paced per book by `STALE_BOOK_S`. A *quiet* book (snapshot held, no deltas) is deliberately NOT recovered — thin-market staleness must keep reading unhealthy so the self-throttle works.
