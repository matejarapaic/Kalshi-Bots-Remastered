...
1. Event ticker: `{SERIES}-{YY}{MON}{DD}{HHMM}{AWAY}{HOME}[G{n}]` — e.g. `KXMLBGAME-26JUL191920LADNYY` = LAD @ NYY, 2026-07-19, 19:20 **ET**. The optional `G{n}` suffix marks doubleheader game n (verified live: `...TBBOSG2`). Field findings from the first live run (2026-07-17): Kalshi uses `AZ` (not ARI) and `CWS` (not CHW) — the full 30-team MLB table has since been validated against all open Kalshi events (44/44 parse, 30/30 teams observed). Market ticker: event ticker + `-{TEAM}` where TEAM is the YES side (`...-NYY` → YES = Yankees win). Team fields are variable-length Kalshi abbreviations; parse by anchoring the 9-char date-time block (`26JUL1919 20` → `YYMONDDHHMM`) and matching the trailing team string against the league's known Kalshi abbreviations, **longest-suffix-first** (HOME is the suffix, AWAY is what remains). If the remainder doesn't split into exactly two known abbreviations → `TickerGrammarError`.
...
5. Exactly one candidate → `MatchResult(market=..., method="alias_exact", ambiguous=False)`, **provided** the parsed ticker start time is within `TIE_BREAK_WINDOW_MIN` of the ESPN start; otherwise `market=None, note="start-time-outside-window"`. (Spec change, field-verified 2026-07-17: after a doubleheader's game 1 settles, only the G2 market remains open — a lone candidate is not automatically the right one. Both sides are scheduled times, so a tight window is safe.)
