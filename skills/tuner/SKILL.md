...
**Session-scoped (owner-directed 2026-08-02):** tuner adjustments never survive a restart. `Tuner.reset()`, called at every orchestrator start, discards whatever overrides the previous session persisted — relaxed *and* tightened, including a raised sigma floor — and zeroes the streak counters and sigma session memory, so every session begins at the human-approved `risk_management.py` baselines. This replaces the earlier restart-re-apply behavior (`Tuner.reload()`, removed). It bounds how long a relax-past-baseline excursion can compound (one process lifetime), but note the flip side: a restart mid-loss-streak also throws away loss-driven tightening — the system comes back up at full baseline sizing with the sigma floor back at its baseline. There is no end-of-session hook because none is needed: overrides live only in risk-management module memory and die with the process; the startup reset is the crash-safe guarantee that the persisted copy is never replayed.
...
Tuner.reset() -> None                                         # startup: discard the previous session's overrides + streaks — every session starts at baseline
...
- **Baseline changed between sessions:** a non-issue since 2026-08-02 — persisted overrides are never re-applied, so a stale value can't collide with an edited baseline; the new session simply starts on the new baselines.
...
- `TestSigmaFloor`: fires at 2 consecutive losses, not 1; raises to session-average vol; raise-only (average at/below current floor → no-op); corridor-capped at 2× baseline; relax (win / no-trade streak) never moves it; windows without a vol reading don't poison the average.
...
- `TestAgent`: `on_reports` persists state; `reset()` returns every parameter to baseline, zeroes streaks and sigma memory, rewrites the state note clean, and announces discarded overrides on Discord (quiet when there was nothing to discard); adjustments notify Discord with the right level.
