...
The tuner skill may adjust parameters at runtime, but only inside a per-parameter corridor whose **ceiling is the human-approved baseline** — live tuning can make the system more conservative than the owner signed off on, never less. The module constants above remain the single source of baselines; overrides are a layer on top, held in module state (`_overrides`) with a bounded audit log (`override_log`, deque maxlen 100).
...
`TUNABLE_BOUNDS` maps each tunable name to `(floor_mult, ceil_mult)` applied to the baseline along the parameter's conservative direction: `ceil 1.0` entries tighten by lowering (sizing/exposure/velocity params, floors 0.25×–0.5×); `(1.0, 2.0)` entries tighten by *raising* (`MIN_EDGE_CENTS`, `EXIT_EDGE_CENTS`, depth minimums, `SIGMA_PLAUSIBLE_MIN`). Integer parameters floor at 1 (count caps can shrink but never hit zero) and integer overrides are stored as ints. Per-skill dict parameters (`SKILL_RISK_MULTIPLIER`, `PER_TRADE_CAP_PCT`, `SKILL_MIN_DEPTH`) take a `skill=` key; `SKILL_RISK_MULTIPLIER` overrides are (live, non_live) tuples validated element-wise. `ENTRY_PHASES` is non-numeric and deliberately not tunable. `RiskOverrideError(RiskError)` is the typed rejection.

**Read-path consequence:** `RiskManager._size_locked`/`_daily_halted` read every tunable via `current()`, and the trader and window-monitor agents now read `MIN_EDGE_CENTS`, `EXIT_EDGE_CENTS`, `STOP_LOSS_PCT`, `SIGMA_PLAUSIBLE_*`, `MIN_DEPTH_WITHIN_5C`, `DEPTH_COLLAPSE_FRACTION` via `current()` at decision time instead of freezing them at import — a live override reaches every consumer on its next evaluation. Non-tunable reads (skill-membership checks, `INTENT_TTL_S`, `ENTRY_PHASES`) stay direct.
