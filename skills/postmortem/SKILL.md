...
- Audit: settled win books derived P&L, closes the trade note (`exit_reason: held_to_settlement`), counts a model hit; entry-condition `False` → violation + `⚠ ENTRY VIOLATION` in the note; model miss → `model✗`.
...
- Declined: signal-log line → known 100-contract held-to-settlement P&L including the real fee formula; zero trades still writes the "watched, nothing traded" line.
