...
2. **Status:** `query(VaultQuery(directory="02-trading-skills", frontmatter_filters={"status": "confirmed"}))` — draft and retired skills are invisible here. **Safety invariant, restated: a `draft` skill can never reach sizing or execution because it never exits this gate.**
