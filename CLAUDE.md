...
8. `discord_bot.py` — trade cards + slash commands behind a swappable transport. Three implementations, in preference order the orchestrator applies automatically: `GatewayTransport` (`kalshi_bots/discord_gateway.py` — a real discord.py websocket connection, receives slash commands and button clicks; runs its own asyncio loop on a background thread) → `DiscordTransport` (REST-only, send but not receive, if the gateway can't connect) → `ConsoleTransport` (local log only, if no Discord credentials are set). `discord_gateway.py` does a lazy/optional import so the rest of the system runs fine without `discord.py` installed. Execution mode (`manual_approve` vs `autonomous`) is a first-class, owner-decided setting, currently `autonomous` on demo only (enforced at construction time, see gates above). Exits are never approval-gated in either mode. `APPROVER_ROLE_ID` gates buttons and `/halt`/`/resume`, but fails *open* (allows the action) if unset — deliberate, since locking out the only operator over a missing role id is worse than the alternative.
