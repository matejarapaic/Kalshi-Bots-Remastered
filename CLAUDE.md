...
- **2026-07-30: the tuner's relax path is no longer capped at the human-approved baseline** (owner-directed, explicit — the tuner skill note previously said removing this required exactly that). `risk_management._corridor()` now floors the relax direction at each parameter's own domain limit (0 for edge/depth-type quantities) instead of at baseline, and `tuner._relax_value()` keeps stepping every time relax fires rather than stopping once it reaches baseline. Net effect: a long enough winning or no-trade streak can now size positions larger than originally authorized, hold more open positions, tolerate bigger per-position losses before the stop-loss backstop fires, and require less edge to enter than the owner signed off on 2026-07-22. See `skills/tuner/SKILL.md` for the full mechanism. Tighten is unaffected — it's still hard-bounded and one-directional (more conservative than baseline only).
